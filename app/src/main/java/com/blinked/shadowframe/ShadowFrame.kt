package com.blinked.shadowframe

import com.blinked.shadowframe.ShadowFrame
import android.content.Context
import android.content.res.TypedArray
import android.graphics.*
import android.util.AttributeSet
import android.widget.FrameLayout
import com.blinked.radiosamui.R


class ShadowFrame(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    FrameLayout(context, attrs, defStyleAttr) {
    // Shadow paint
    private val mPaint: Paint = object : Paint(Paint.ANTI_ALIAS_FLAG) {
        init {
            setDither(true)
    ...
    var shadowRadius: Float
        ...
        set(shadowRadius) {
            ...
            // Set blur filter to paint
            mPaint.setMaskFilter(BlurMaskFilter(mShadowRadius, BlurMaskFilter.Blur.NORMAL))
            resetShadow()
        }
    ...
    init {
        ...
        val typedArray: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.ShadowFrame)
        try {
            isShadowed = typedArray.getBoolean(R.styleable.ShadowFrame_sl_shadowed, true)
            shadowRadius = typedArray.getDimension(
                R.styleable.ShadowFrame_sl_shadow_radius,
                DEFAULT_SHADOW_RADIUS
            )
            shadowDistance = typedArray.getDimension(
                R.styleable.ShadowFrame_sl_shadow_distance,
                DEFAULT_SHADOW_DISTANCE
            )
            shadowAngle = typedArray.getInteger(
                R.styleable.ShadowFrame_sl_shadow_angle,
                DEFAULT_SHADOW_ANGLE.toInt()
            )
                .toFloat()
            shadowColor = typedArray.getColor(
                R.styleable.ShadowFrame_sl_shadow_color,
        ...
        if (mIsShadowed) {
            ...
            if (mInvalidateShadow) {
                ...
                if (mBounds.width() !== 0 && mBounds.height() !== 0) {
                    ...
                    val extractedAlpha = mBitmap?.extractAlpha() ?: return
    ...
    companion object {
        ...
        private val DEFAULT_SHADOW_COLOR: Int = Color.DKGRAY
